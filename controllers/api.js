...
const store = multer.diskStorage({
  destination(req, file, cb) {
    cb(null, path.join(__dirname, 'uploads/plantImages'));
  },
  filename(req, file, cb) {
    cb(null, `${req.user._id}-${Date.now()}${path.extname(file.originalname).toLowerCase()}`);
  },
});
...
exports.getLastfm = (req, res, next) => {
  ...
  const artistTopTracks = () => new Promise((resolve, reject) => {
  ...
  const artistTopAlbums = () => new Promise((resolve, reject) => {
